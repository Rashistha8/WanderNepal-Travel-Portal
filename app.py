...
from models import db, User, Destination, Booking  # make sure models.py is correct

# ------------------------------
# Top-level Flask app variable
# ------------------------------
app = Flask(__name__)
...
def book(dest_id):
    ...
    if request.method == 'POST':
        ...
        booking = Booking(user_id=user_id, destination_id=dest_id, travel_date=travel_date, status='pending')
        db.session.add(booking)
        db.session.commit()
        booking.status = 'confirmed'  # Simulate payment success
        db.session.commit()
...
def admin_confirm_booking(booking_id):
    ...
    if booking.status == 'pending':
        ...
        db.session.commit()
        flash(f'Booking ID {booking.id} confirmed!')
    else:
        flash(f'Booking ID {booking.id} already confirmed.')
    return redirect(url_for('admin_bookings'))

# ------------------------------
# Note:
# Vercel automatically looks for 'app' variable at top-level
